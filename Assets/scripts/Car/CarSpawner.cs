...
public class CarSpawner : MonoBehaviour
{
    ...
    private Vector3 _zoneAfterCarForSpawnCars;
    private int _countCarsForSpawn = 0;

    private void Start()
    {
        _zoneAfterCarForSpawnCars = new Vector3(_road.transform.position.x - _road.transform.localScale.x / 2 - _beforeRoadDistance,
    ...
    private IEnumerator SpawnCar()
    {
        if (_countCarsForSpawn != 0)
        {
            ...
            GameObject currentCar = Instantiate(_carPrefab, _zoneAfterCarForSpawnCars, _carPrefab.transform.rotation);
